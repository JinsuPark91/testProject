...
const { Item } = List;
const MAX_THUMB_COUNT = 4;

function RoomItem({ room, underLine, onClick, selected }) {
  const thumbs = [
  ...
  return useObserver(() => (
    <Wrapper underLine={underLine}>
      <StyledItem onClick={onClick} selected={selected}>
        <Item.Meta
          avatar={<Photos srcList={thumbs} maxCount={MAX_THUMB_COUNT} />}
          title={
            <Title>
              <RoomNameText>{room.name}</RoomNameText>
              <UserCountText>{room.users.length}</UserCountText>
            </Title>
          }
          description={<StyleRoomMessage>{room.lastMessage}</StyleRoomMessage>}
        />
        {room.unreadCount * 1 ? (
